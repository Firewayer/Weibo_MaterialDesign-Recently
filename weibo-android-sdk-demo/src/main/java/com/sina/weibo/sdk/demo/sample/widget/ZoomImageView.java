...
public class ZoomImageView extends ImageView {
    ...
    private IZoomViewListener iZoomViewListener;
    private boolean isClick;
    private boolean isFingers;
     ...
     */
    public void setImageBitmap(Bitmap bitmap, IZoomViewListener iZoomViewListener) {
        sourceBitmap = bitmap;
        this.iZoomViewListener = iZoomViewListener;
        invalidate();
    }
        ...
        switch (event.getActionMasked()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (event.getPointerCount() == 1) {
                    ...
                    if (movedDistanceX == 0 && movedDistanceY == 0 && !isFingers) {
                        isClick = true;
                    }
                    // 进行边界检查，不允许将图片拖出边界
        ...
        if (sourceBitmap != null) {
            ...
            if (bitmapWidth > width || bitmapHeight > height) {
                ...
                if(width - bitmapWidth < height - bitmapHeight){
                    Log.d("TAG", "图片初始化成功");
                    // 当图片宽度更接近屏幕宽度时，将图片等比例拉伸，使它可以完全显示出来
                    float ratio = width / (bitmapWidth * 1.0f);
                    matrix.postScale(ratio, ratio);
                    float translateY = (height - (bitmapHeight * ratio)) / 2f;
                    // 在纵坐标方向上进行偏移，以保证图片居中显示
                    ...
                    Log.d("TAG", "初始化高度");
                    // 当图片高度更接近屏幕高度时，将图片等比例拉伸，使它可以完全显示出来
                    float ratio = height / (bitmapHeight * 1.0f);
                    matrix.postScale(ratio, ratio);
                    float translateX = (width - (bitmapWidth * ratio)) / 2f;
                    // 在横坐标方向上进行偏移，以保证图片居中显示
