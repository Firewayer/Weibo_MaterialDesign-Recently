...
public class StatusList {
    ...
    public static StatusList parse(String jsonString) {
        ...
        try {
            ...
            JSONArray jsonArray      = jsonObject.optJSONArray("statuses");  //jsonObject中取出一个以“statuses”开头的JSONArray
            if (jsonArray != null && jsonArray.length() > 0) {
                int length = jsonArray.length();
                statuses.statusList = new ArrayList<Status>(length);
                for (int ix = 0; ix < length; ix++) {
                    statuses.statusList.add(Status.parse(jsonArray.getJSONObject(ix)));     //将存储在JSONArray中的元素数组中每一个元素解析出来，并通过Bean类的parse方法，直接将其解析为Status对象存到ArrayList<Status>中
                }
            }
